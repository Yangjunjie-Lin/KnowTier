...
`1.0.0-rc.3` (the Python lock uses its normalized PEP 440 spelling). Update the changelog and
privacy notice when their contents change, commit the release state, then create
`v1.0.0-rc.3` from that reviewed commit. Release tags are immutable: if a candidate tag already
