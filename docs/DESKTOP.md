...
The current build is `v1.0.0-rc.4`. It is a release candidate and remains a GitHub Draft Release;
...
```powershell
Get-FileHash .\KnowTier-Setup-1.0.0-rc.4-windows-x64.exe -Algorithm SHA256
```
...
```bash
chmod +x KnowTier-1.0.0-rc.4-linux-x64.AppImage
./KnowTier-1.0.0-rc.4-linux-x64.AppImage
```

For Debian or Ubuntu, install the package with `sudo apt install ./knowtier_1.0.0-rc.4_amd64.deb`.
