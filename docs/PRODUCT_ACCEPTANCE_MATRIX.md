...
| Desktop first run | Sidecar boot, authenticated ready handshake, onboarding, Mock full flow, App Data persistence | local FastAPI on random `127.0.0.1` port | App Data SQLite/uploads/logs; rehydratable semantic projection | packaged sidecar/lifecycle/install smoke | PARTIAL: sidecar smoke passed; native installer smoke awaits hosted build |
